...
public class ProgramDashboardServices {
    ...
    public void initializeDashboard(IStmt programStatement, String logFilePath) {
        ...
        PrgState prgState = new PrgState(new ExeStack(), new SymTable(), new OutList(), new FileTable(), new Heap(), programStatement, new LockTable());
    ...
    private void updateLockTable() {
        ObservableList<Map.Entry<Integer, Integer>> lockTableData = FXCollections.observableArrayList(
                controller.getPrgList().getFirst().getLockTable().getContent().entrySet()
        );
